...
### 4/27 audit
- 시작 시각: 2026-04-27 09:35 KST (월요일)
- 종료 시각: 2026-04-27 09:46 KST
- 트리거: 사용자 `/tirtir-audit` skill, "organize tasks for the TIRTIR campaign"
- 주간 경계: today=4/27(Mon) → 이번주 4/27(월)~5/3(일), 다음주 5/4(월)~5/10(일)
- 4/22 audit + 4/24 봇 EOD 자동 발송 이후 5일/3일 경과
- 신규 발견:
  - **R209 Meta Ads Audience Segments → Complete** (이동호 4/25 15:46 직접 설정 완료. engaged audience = official store_visit_180d / existing customers = official store_purchase_180d. 하지현 대행 불필요)
  - **R68 Matcha Giveaway 종료/롤백 → Complete** (이동호 4/25 16:01 client channel 종료 확인. Shopify 메인배너/딜페이지/Matcha Refresh 하단 배너 OFF 완료)
  - **R203 Google MC 제품매칭 진단 → Processing** (황아인 4/24 16:40 1차 정리 공유, 이동호 검토 진행)
  - **신규 3건**:
    - R210 Matcha Refresh 4/27 종료 - 광고 OFF (홍유니, today KST 15:00)
    - R211 Matcha Refresh 4/27 종료 - Shopify 롤백 (이동호, 4/28 KST 16:00)
    - R212 5/15 AI 샤셰 1개 우선 리스팅 + 썸네일 깨짐 확인 (이동호/황아인, 4/28)
  - **운영 규칙 변경 1건 (RC8)**: Glow Gift Festival 할인율/운영기간 swap (자연님 4/23 20:03 client 명시) → 운영기간 5/1~5/29 (장기), 4단 차등 → 3단 단순화 (20%/25%/30%). Shopify(R176)/광고(R179)/Klaviyo(R178) 전반 재반영 필요
  - **추가 운영 규칙**: Klaviyo 레퍼 사진 사용 제한 (자연님 4/23 19:21) — 핀터레스트 등 외부 레퍼 그대로 사용 금지(저작권). 이후 참고용만
  - **TODAY P0 (4/27)**: Primer Launch Deal 4/27 라이브 (KST 13:00) + Matcha Refresh 광고 OFF (KST 15:00) + 4/28 Shopify 롤백 (KST 16:00) 동시 진행
- **gws CLI 인증 만료 — 시트 batchUpdate 불가** (refresh token expired). Drive MCP로 read-only. 시트 변경사항(R209/R68 Complete, R203 Processing, R210/R211/R212 신규 + R174/R175/R176/R179 Description 업데이트)은 사용자 수동 적용 또는 다음 audit 시 인증 복구 후 일괄 적용 필요
- RC 위반: 1건 (RC20 사전 감지)
  - R21/R126 (둘 다 시트 Status=Complete)을 초기 draft 지연 섹션에 포함 → STEP 6 RC20 status 재조회에서 즉시 감지 + 발송 전 제거. 발송 전 예방 성공
- 발송 ts: 1777250767.026369 (C0ALWLLQFU7 내부 채널, 확인 없이 자동 발송)
- RC21 grep CLEAN
