...
### 5/1 09:36 audit
- 시작 시각: 2026-05-01 09:36 KST (금요일)
- 종료 시각: 2026-05-01 09:48 KST
- 트리거: 사용자 `/tirtir-audit` skill 호출, "organize tasks for the TIRTIR campaign"
- 주간 경계: today=5/1(Fri) → 이번주 4/27(월)~5/3(일), 다음주 5/4(월)~5/10(일)
- 4/22 audit 이후 9일 경과 (4/27, 4/28, 4/29, 4/30 봇 EOD 자동 발송 — 본 audit이 manual scan 첫 재개)
- **시트 write 불가**: gws CLI auth invalid_grant — refresh token 폐기. Google Drive MCP는 read-only. **시트 변경 사항 7~8건은 다음 audit에서 auth 복구 후 일괄 반영 예정** (R158/R159/R160/R162/R163/R164/R209 → Complete 후보, R175 진행 단계, 신규 8건 미append)
- 신규 발견 (4/22 audit ~ 4/30 EOD 누적):
  - **Primer Launch Deal 4/27 라이브 → Complete** (R158/R159/R160/R162/R163/R164) — 4/27 EST 00:00 = KST 13:00 라이브 완료. 단품 20% / 번들 25%, 하드번들 3종 + 프라이머 hover 적용
  - **Spring Matcha Refresh 4/28 종료/롤백 → Complete** (R142, R155 별도) — 광고 OFF 4/28 15:53 / Shopify 롤백 16:15. Matcha Clean Duo 번들 비공개 처리
  - **Spring Matcha Klaviyo 발송 → Complete** (R138) — 4/21 17:47 + AI 최적시간 + 4/22 Earth Day Drop 발송 완료
  - **Meta Ads Audience Segments 세팅 → Complete** (R209) — 이동호 4/25 직접 세팅. 하지현 대행 불필요
  - **Matcha Giveaway 4/24 종료 → Complete** (R103 + 광고/배너/이벤트페이지 OFF)
  - **Glow Gift Festival 운영기간/할인율 SWAP** — 자연님 4/23 20:03 ts 1776942192. 5/1~5/29 (29일 장기화), 3단(20%/25%/30%) 단순화. R175~R180 sub-task 재반영
  - **Glow Gift Festival Meta 광고 세팅 → Complete** (R180 부분) — 홍유니 4/30 13:53 메타 세팅 완료. NL/UK 메세지 카드 재고 없음 → 제외 진행
  - **Glow Gift Festival Google 광고 세팅 → 진행 단계** — 홍유니 4/30 21:02 세팅 완료, 페이지 비활성화로 반려 → 5/1 KST 13:00 페이지 오픈 후 재검토→라이브
  - **Primer Launch Deal 2차 Klaviyo 발송 → Complete** — 이동호 4/30 18:03 발송 완료
  - **4/30 TIRTIR VC 미팅 → Complete** (R198 추정) — 황아인 4/30 17:59 회의록 공유
  - **GLOW BASE SET 번들 매칭 오류** — 황아인 4/29 12:31 발견 (빨간 픽서 매칭 → 분홍 픽서로 잘못 매칭, 5건 영향 1건 발송/4건 미처리). 노출 중인 번들 즉시 수정. 핑크 픽서 가격 $18.73→$22 ($3.27 초과) → 기주문 4건 할인 발송 제안 → TIRTIR 자연님/사라님 컨펌 대기
  - **신규 client 요청 2건 (4/30 자연님)**:
    - AO 광고 개선 계획안 전달 (홍유니, ~5/7) — 5월 AO+SEO/AEO 개선으로 세션 회복 집중
    - shop all/Make up/skin care 메뉴 정렬 작업 (이동호, ~5/6) — 프라이머 기준 크기 정렬 + 베스트셀링순 + New/Sale 뱃지 + Sold out 후순위
  - **신규 운영 규칙 3건**:
    - 소재 기획 → 시안 컨펌 프로세스 단순화 (홍유니 4/29 19:06 ts 1777457201): 디자인 시안 후 디테일업/싱크용 추가 수정만 허용, 오브제 변경/요청 회수/의사결정 번복 지양
    - Klaviyo SHOP NOW 버튼 컨벤션 (황아인 4/28 17:27): 4/28 이후 캠페인부터 이미지 형태로 제작 (폰트 12+Bold 우선)
    - Klaviyo 외부 레퍼런스 사진 그대로 사용 금지 (자연님 4/23 19:21): 참고용으로만 활용
    - UTM 통일 규칙 (이동호 4/22 20:47): 모든 캠페인 Tracking Template = Final URL Suffix 방식. Google 매체 전부 적용
  - **신규 task 8건 식별 (sheet 미append, auth 복구 후 추가 예정)**:
    - 5월 미디어믹스 시트 양식 업데이트 (황아인, today 5/1)
    - Meta EU top7 언어 테스트 (황아인, ~5/2)
    - Pmax 추가 텍스트 에셋 그룹 (김소형, ~5/2)
    - AI Filter Cushion Sachet 5/15 프로모션 진행사항 데일리 공유 (김소형, daily)
    - Glow Gift Festival 5/30 종료 - Shopify 롤백 (이동호, 5/30 KST 16:00)
    - Glow Gift Festival 5/29 광고 OFF (홍유니, 5/29 KST 15:00)
    - Pore Blur Set US 핑크→레드 픽서 노출 변경 (이동호, today 5/1)
    - GLOW BASE SET 번들 세팅 크로스체크 SOP 문서화 (이동호+황아인)
  - **Today (5/1) D-day**: Glow Gift Festival 5/1 라이브 KST 13:00 / Glow Gift 1차 Klaviyo / Google 페이지 활성화 후 라이브 / GLOW BASE SET 픽서 답변 / 4월말 Tasks 도출 (1일 지연 carry-over)
- **RC 위반: 1건 (RC9 외부 요인)**
  - 시트 batchUpdate 자체 불가 — gws CLI refresh token invalid_grant. Read는 Google Drive MCP로 우회 가능했으나 write 경로 없음. 데이터 손상은 0건. **다음 audit에서 auth 복구 후 carry-over 변경사항 일괄 반영 예정**
- 발송 ts: 1777596467.580509 (C0ALWLLQFU7 내부 채널, 확인 없이 자동 발송)
- 발송 본문은 시트 Status 의존 없이 슬랙 thread/EOD 이벤트 기반 서술로 작성 — 시트 미반영 변경사항이 메시지 정확성에 영향 없음
