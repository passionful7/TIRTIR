...
### 5/13 09:15 audit
- 시작 시각: 2026-05-13 09:15 KST (수요일)
- 종료 시각: 2026-05-13 10:05 KST
- 트리거: 사용자 `/tirtir-audit` skill 호출, "run the TIRTIR audit skill to organize tasks for the TIRTIR campaign"
- 주간 경계: today=5/13(Wed) → 이번주 5/11(월)~5/17(일), 다음주 5/18(월)~5/24(일), 다다음주 5/25(월)~5/31(일)
- 4/22 11:36 audit 이후 약 21일 경과 (그동안 5/11 EOD + 5/12 EOD bot 메시지로 자동 보완됨 — manual audit은 4/22가 마지막)
- 5/12 09:35 bot EOD가 직전 status snapshot이며, 본 audit는 그 이후 ~24시간 변경사항 incremental 처리
- 신규 발견 (5/12 09:35 EOD 이후):
  - **Glow Gift Festival 딜 페이지 구성품 네임 오류 수정** — 황아인 5/12 11:28 ts 1778552894 "수정 반영 완료하였습니다!" + 김소형 권역별 PC/MO 4건 교체 (US/KR/UK/NL) → **Complete** (시트 미반영)
  - **신규 task 4건 (5/12 18:43 V포토북 + Summer Big Sale 유선 결정사항 from 황아인 thread ts 1778579017)**:
    - V포토북 5/18 V에셋 (Klaviyo 삽입) 수신 + 반영 (이동호, 5/19)
    - V포토북 Klaviyo E_0~E_1b 디자인 제작 (김소형, 5/21)
    - V포토북 5/21 테스트 페이지 전달 (이동호, 5/21)
    - Summer Big Sale 메타 소재 5종 + IG 스토리 + 구글 애셋 기획 (김소형 → 디자인팀 Heewon, 5/13 14:00 KST)
  - **신규 task 1건 (5/12 11:04 매체 최적화 from 하지현 thread ts 1778551471)**:
    - 티르티르 매체 최적화 액션 아이템 정리/검토 (하지현, 5/16) — 하지현 5/12 17:51 시트 gid=1285823502에 1차 의견 등록
  - **Today P0 (5/13)** carry-over from 5/12 EOD:
    - R225 Summer Big Sale 6/1 슬라이드 3,4 추가 질문 답변 (이동호)
    - R232 Shop all/Make up/Skin care 메뉴 정렬 (이동호, today Due)
    - R17 카탈로그 피드 / R42 GMC 피드 답변 (이동호, 5/13 약속)
    - AI Sachet 1차 말차 라인 5/13 시안 전달 (김소형)
- **시트 batchUpdate 불가 (제약사항)**:
  - gws CLI auth refresh token expired (invalid_grant: Bad Request) → 시트 직접 수정 불가
  - 신규 5건 (V포토북 4개 + Summer Big Sale 디자인 1개 + 매체 최적화 1개) + Glow Gift 딜페이지 완료 1건 모두 시트 미반영
  - 사용자에게 보고 필요: refresh token 재발급 시 시트 task 추가 필요
- RC 위반: 1건 (RC9 변형 — gws auth 만료, 시트 batchUpdate skip. 시트 반영은 사용자 인지/조치 후 재시도 필요)
- 발송 ts: 1778631872.637729 (C0ALWLLQFU7 내부 채널, 확인 없이 자동 발송)
- 발송 본문 grep: CLEAN (CLAUDE.md/skill/AUDIT_LOG/RC[0-9]+/META-RULE/self-correction 매치 0건)
