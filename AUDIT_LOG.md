...
### 4/30 12:24 audit
- 시작 시각: 2026-04-30 11:50 KST (수요일)
- 종료 시각: 2026-04-30 12:24 KST
- 트리거: 사용자 `/tirtir-audit` skill 호출, "organize tasks for the TIRTIR campaign"
- 주간 경계: today=4/30(Wed) → 이번주 4/27(월)~5/3(일), 다음주 5/4(월)~5/10(일)
- 4/22 audit 이후 8일 경과. 그 사이 봇 자동 EOD 5건 발생 (4/22, 4/24, 4/27, 4/28, 4/29) — 본 audit이 4/22 이후 첫 manual audit
- 환경적 제약: **gws CLI auth 만료** (`invalid_grant`) — 시트 직접 업데이트 불가. Slack 메시지를 진실의 원천으로 발송, 봇 자동 EOD 또는 수동 입력으로 reconciliation 필요
- 신규 발견:
  - **Complete 8+건**: R157~R164 Primer Launch Deal 일괄 (4/27 라이브 검증) / R68 Matcha Giveaway 종료 (4/25) / R185 자사몰 하단 영상 (4/22) / R200 Spring Matcha GWP UK/EU 분리 (라이브 후 운영 중) / R204 TIRTIR 영문교육자료 (4/23) / R205 딜페이지 영상소스 (4/23) / R208 Glow Base 번들 SKU 답변 / R209 Meta Audience Segments (이동호 4/25 직접 세팅, 하지현 대행 불필요)
  - **신규 4건**:
    - 미디어믹스 5월 시트 양식 업데이트 (황아인, 4/30 today) — 홍유니 4/29 12:06 요청
    - Meta EU top7 언어 테스트 (황아인, ~5/2) — 홍유니 4/29 11:43
    - Pmax 추가 텍스트 에셋 그룹 (김소형, ~5/2) — 홍유니 4/29 11:47
    - AI Filter Cushion Sachet daily 진행사항 공유 (김소형, daily) — 홍유니 4/29 11:17
  - **CRITICAL 신규 이슈**: GLOW BASE SET 번들 오류 (4/29 12:31 황아인 발견)
    - 빨간 픽서 → 분홍 픽서 매칭 오류, 5건 주문 영향 (1건 발송 / 4건 미처리)
    - 픽서 가격 변경 제안 ($18.73→$22, $3.27 초과) — TIRTIR 컨펌 대기
    - 5/1 재발방지 안내+사과 client 공유 예정 (홍유니 4/29 18:59 약속)
    - 내부 번들 세팅 크로스체크 SOP 정립 필요
  - **미생성 시트 task 다수**: R210~R214 (봇이 4/27~4/28 EOD에서 언급) / Glow Gift Festival 5/30 종료/롤백 / Primer 종료/롤백 / AI Filter Cushion Sachet 9 sub-task
  - **운영 규칙 변경 1건**: 소재 기획→시안 컨펌 프로세스 단순화 (홍유니 4/29 19:06 ts 1777457201). 시안 디자인 컨펌 1회 sync로 종결, 회수/번복 지양
  - **Due 변경 0건** (모든 지연 task는 N일 지연으로 표기, 임의 변경 없음)
- **RC 위반: 0건** (RC1~RC22 패턴 위반 감지 없음)
- 시트 업데이트: **불가** (gws auth 만료). Slack 메시지를 카논으로 사용. 봇 자동 EOD 또는 수동 입력으로 사후 reconciliation 필요
- 발송 ts: 1777508687.257609 (C0ALWLLQFU7 내부 채널, 확인 없이 자동 발송)
