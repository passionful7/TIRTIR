...
| # | 단계 | 체크 항목 | RC | 4/7 19:30 | 4/8 11:30 | 4/13 13:00 | 4/15 14:00 | 4/16 15:30 | 4/17 11:22 | 4/22 | 5/7 09:15 |
|---|------|-----------|-----|-----------|-----------|------------|------------|------------|------------|------|
...
## 진행 노트 추가 (5/7 09:15 audit)

### 5/7 09:15 audit
- 시작 시각: 2026-05-07 09:15 KST (목요일)
- 종료 시각: 2026-05-07 09:43 KST
- 트리거: 사용자 `run the TIRTIR audit skill to organize tasks for the TIRTIR campaign`
- 주간 경계: today=5/7(Thu) → 이번주 5/4(월)~5/10(일), 다음주 5/11(월)~5/17(일), 다다음주 5/18(월)~5/24(일)
- 4/22 audit 이후 약 15일 경과 (그 사이 5/5, 5/6 자동 EOD 봇 메시지만 발송됨, manual audit 미실행)
- 신규 발견 요약:
  - **신규 운영 합의 1건**: 메타 상시 광고 cadence — *2주 1회 텍스트 에셋 교체 / 6월부터 이미지 에셋 교체* (자연님 4/30 11:12 → 5/6 18:56 추가 협의 → 홍유니 5/6 20:35 절충안 합의). BDRN 일괄 OFF 합의
  - **5/7 08:00 4월 정산서 client 전달 완료** (홍유니 ts 1778108404) — 대행수수료(소매출-매체비)*4% + 광고비 청구 분리, 4월평균 환율 적용. 김현서 확인 후 황아인이 세금계산서 발행 예정
  - **Primer Launch Deal 운영기간 확정** 4/27~5/27 (김소형 5/6 20:49 ts 1778068164) — 종료/롤백 task 신규 생성 필요 (5/28 KST 16:00 close)
  - **Glow Gift Festival 운영기간** 5/1~5/29 (자연님 4/23 20:03 ts 1776942192) — 종료/롤백 task 신규 생성 필요 (5/30 KST 16:00 close)
  - **5/15 AI Cushion Sachet Event** 라이브 9개 sub-task 분해 필요 (자연님 4/28 17:41 기획안 ts 1777365717 + 김소형 5/6 20:21 디자인 1차 기획안 ts 1778066491 피그마)
  - **상태 변경 12건 식별 (시트 update 필요)**: R157~R163 Primer 4/27 라이브 sub-task 모두 Complete, R166 PDP 수정 Complete, R173~R178/R180 Glow Gift 5/1 라이브 sub-task 모두 Complete, R181 Spring Matcha 종료 4/28
  - **인플루언서 소재 사용 가능 확인** (김소형 5/6 21:56 ts 1778072180) — Rebecca Brewer/khushhyyy 활용 가능, R190/R191 unblock → 다음주 운영
  - **레버 엑스퍼트 raw data sheet 공유** (이동호 5/6 22:31 ts 1778074284) — 환율 시트 별도, Klaviyo 통합 시트 일원화 정책 (5/4 12:46 ts 1777862813)
- **시트 업데이트 BLOCKED**: gws CLI Google OAuth refresh token 만료/취소(`invalid_grant: Token has been expired or revoked`). 시트 batchUpdate 미실행. 본 audit은 status 식별 + Slack 발송까지만 완료. 사용자 측 재인증 후 sheet 적용 필요
- **RC 위반 / 예방 1건**:
  - RC21 사전 감지: Draft 본문에 "(RC8 운영 규칙)" 표현 1건 → grep 단계에서 즉시 제거 후 재grep CLEAN 확인 후 발송. 발송 전 예방 성공 (사용자 지적 없이 자체 발견)
- 발송 ts: 1778113418.518269 (C0ALWLLQFU7 내부 채널, 확인 없이 자동 발송)
- 사용자 액션 필요:
  - Google OAuth 재인증 → 시트 status update 적용 (R157~R181 Complete 처리, R191 Result 업데이트, R165 Result 업데이트)
  - 신규 task 16건 시트 append: Primer/Glow Gift 종료 4건 + 5/15 AI Sachet 6건 + 4월 정산/AO/Klaviyo R&R/AI 비디오 검토/메타 상시 소재 6건
