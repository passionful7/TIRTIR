...
### 5/18 audit
- 시작 시각: 2026-05-18 11:00 KST (월요일)
- 종료 시각: 2026-05-18 11:35 KST
- 트리거: 사용자 `/tirtir-audit` skill 호출, "organize tasks for the TIRTIR campaign"
- 주간 경계: today=5/18(Mon) → 이번주 5/18(월)~5/24(일), 다음주 5/25(월)~5/31(일)
- 4/22 audit (이전 AUDIT_LOG column) 이후 약 26일 경과. 단, 5/14 09:18에 매도비 봇이 audit-style 메시지를 채널에 게시한 흔적 발견(이전 claude 세션 추정) — 본 audit은 5/14 이후 4일분 incremental 형식으로 작성
- 환경 제약 (CRITICAL):
  - **gws CLI OAuth 인증 만료**: refresh_token `invalid_grant`. 시트 직접 batchUpdate 불가 → 발견 사항은 Slack 메시지에 집결, 사용자 측 수기 반영 또는 token 갱신 후 일괄 반영 필요
  - sheet_dump.json(5/13 자) + Google Drive MCP `read_file_content`(5/18 현재) 결합으로 시트 상태 확인. Drive 마크다운 렌더링은 복수 탭이 섞여 있어 `업무` 탭은 행 약 0~235까지로 파싱
  - Slack chat.postMessage는 봇 토큰 API 정상 작동, MCP Slack 읽기 도구도 정상
- Sub-agent 3건 병렬 실행:
  1. 고객사 채널 C0AMP96KZHB 4/22~5/18 전수 스캔
  2. 내부 채널 C0ALWLLQFU7 4/22~5/18 전수 스캔
  3. RC19 active-task thread 11건 양 채널 재독
- 5/14~5/18 incremental 신규 발견:
  - **Complete 1건**: 자동 발송 메일 1차 수정 (주문 지역 배송 시작 메일 live chat URL, 황아인 5/15 14:25 ts 1778739317.543289)
  - **신규/업데이트 5건**:
    - Primer Launch Deal 번들 제품 추가 (Glow Primer+Red Cushion / Pore Primer+Red Cushion $37.60 20%) (황아인 5/15 18:02)
    - Klaviyo 4차 발송 5/21 → 5/19 AM 단축 (디자인 김소형 / 발송 김현서)
    - 할인율 1차 조정 5/20~30 — AI 20%, 컨실러 20% (자연님 5/14 11:01)
    - 할인율 2차 조정 6/17~30 — AI 28%, 컨실러 20% (자연님 5/14 11:01)
    - ACA Retargeting 크리에이티브 개선 문의 (자연님 5/14 11:07 / 박서우 5/15 추가) — 답변 대기
    - 메타 광고 성과 우려 문의 (자연님 5/14 10:21) — 황아인 5/14 ROAS 상시 단독 운영 전환 답변, 박서우 5/15 학습 초기화 문의 답변 대기
- RC 위반: 0건 (사전 예방 — grep CLEAN 1차 통과)
- 발송 ts: 1779064115.658759 (C0ALWLLQFU7 내부 채널, 확인 없이 자동 발송)
- 본 audit의 사이드 effect: AUDIT_LOG.md 자체가 git 저장소에 commit/push되어 PR #22(draft)로 생성됨 — 리포지토리는 `passionful7/tirtir`, 브랜치 `claude/sweet-clarke-TF0sL`
