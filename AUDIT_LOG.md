...
### 5/14 audit
- 시작 시각: 2026-05-14 00:03 KST (목요일)
- 종료 시각: 2026-05-14 00:45 KST
- 트리거: 사용자 `/tirtir-audit` skill 호출, "run the TIRTIR audit skill to organize tasks for the TIRTIR campaign"
- 주간 경계: today=5/14(목) → 이번주 5/11(월)~5/17(일), 다음주 5/18(월)~5/24(일), 다다음주 5/25(월)~5/31(일)
- 4/22 audit 이후 22일 경과 (긴 윈도우)
- Sub-agent 2건 병렬 실행 (client + internal 채널 4/22~5/14 22일치 스캔)
- 신규 발견 — 완료 후보:
  - **R185 자사몰 하단 영상 3건** → Complete 후보 (황아인 4/22 18:38 반영 완료)
  - **R226 메타 정산 VAT 답변** → Complete 후보 (이동호+홍유니 5/11 13:08 답변 완료)
- 신규 발견 — 메가 이슈:
  - **Summer Big Sale 6/1~6/10 신규 프로모션** (자연 5/4 ts 1777538236 / 박서우 5/8/5/11 기획안 확정) — 11+ sub-task 필요 (메인 딜배너 5/18, V포토북 V에셋 5/18, V포토북 Klaviyo 디자인 5/21, 테스트 페이지 5/21, 5/25 사전 라이브 7건 5/21 디자인 + 5/22 세팅, V포토북 사전 예약 5/24, 메타 5종+IG+구글 애셋 5/15, Shopify 세팅 5/29, 광고 캠페인 5/30, 6/1 라이브, 6/11 종료/롤백)
  - **메타 결제 방식 변경** (5/13 17:28 홍유니): 5/31 마지막 매드업 카드 결제 → 6/1 티르티르 직접 결제 전환
  - **6월 매출/광고예산 변경** (자연 5/13 14:38): 6월 매출 ₩552M / 광고 ₩59.42M (7월→6월 당김)
  - **AI Sachet 5/15 HOLDING** (자연 5/8 ts 1777365717 + 박서우 5/11 17:47 재확정)
  - **운영 규칙 10건 변경**: UTM Final URL Suffix 통일(4/22) / Klaviyo 레퍼 저작권(4/23) / Klaviyo SHOP NOW 이미지(4/28) / 소재 7단계 컨펌(4/29) / AO=Always On 정의(4/30) / Klaviyo 시트 일원화(5/4) / Klaviyo Flow R&R 분기 - Retentics(5/4) / 상시광고 2주 cadence(5/6) / GMC 배송 $15/$40(5/8) / 메타 결제 전환(5/13)
  - **이미 완료된 주요 이벤트** (slack 확인): Spring Matcha Refresh 종료(4/28), Matcha Giveaway 종료(4/25), Primer Launch Deal 라이브(4/27), Glow Gift Festival 라이브(5/1), Lever Xpert 대시보드 공식 공유(5/7), 4월 매체 정산서/세금계산서(5/7~5/8), Meta Audience Segments R209(4/25), 베스트셀러 변경 1차 적용(5/7~5/8), KR OOS 3건 처리(5/7~5/8)
- **RC 위반 / 차단**:
  - ⚠️ OAuth refresh token 영구 invalidated (Google `invalid_grant`): gws CLI 시트 batch update 일시 불가. 모든 변경사항은 Slack 메시지 본문에 명시하여 수기 반영 + 토큰 재발급 후 일괄 적용 가능하도록 처리.
  - RC21 사전 감지 1건: draft 본문에 "CLAUDE.md 반영 필요" 표현 발견 → grep 단계에서 즉시 제거 → 재grep CLEAN 후 발송. 발송 전 예방 성공.
- 시트 batch update: **불가능** (OAuth). 변경 후보 (R185 Complete + R226 Complete + Summer Big Sale 신규 11+ sub-task + 신규 단일 태스크 다수) 모두 Slack 메시지 본문에 명시.
- 발송 ts: 1778717937.072249 (C0ALWLLQFU7 내부 채널, 확인 없이 자동 발송, draft 12.7KB)
- 후속: 사용자에게 (1) Google OAuth 재인증 (2) Slack 메시지 기반 시트 수기 반영 또는 토큰 재발급 후 일괄 sync 요청 필요
