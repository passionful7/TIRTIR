...
### 5/8 09:00 audit
- 시작 시각: 2026-05-08 ~09:00 KST (금요일)
- 종료 시각: 2026-05-08 09:19 KST 발송
- 트리거: 사용자 `/tirtir-audit` skill 호출
- 주간 경계: today=5/8(Fri) → 이번주 5/4(월)~5/10(일), 다음주 5/11(월)~5/17(일), 다다음주 5/18(월)~5/24(일)
- 4/22 audit 이후 16일 경과 (그 사이 봇이 5/4, 5/5, 5/6, 5/7 daily EOD 자동 발송 → 본 audit은 5/7 09:23 EOD 이후 변경사항 + 미반영 신규 task 정리에 집중)
- **gws CLI 인증 broken (invalid_grant)** → Google Sheet 직접 write 불가. Slack 메시지는 정상 발송. **사용자 액션 필요: `gws auth login` 재인증 후 시트에 신규 task 직접 추가** (Primer/Glow Gift 종료 4건 + 5/15 AI Sachet sub-task 8건 + 프라이머 후속 번들 등)
- Sheet read는 Google Drive MCP `download_file_content` (CSV export)로 우회 — 208 rows, Status 분포 Complete 163 / Processing 18 / Not yet 26
- Slack 스캔: Drive 다운로드 + Slack MCP slack_read_channel + sub-agent 병렬 deep-read (4/22~5/8 양 채널 + bot EOD 4건 thread)
- 5/7 09:23 EOD 이후 신규 발견:
  - **Lever Xpert 대시보드 client 공유** (이동호 5/7 13:01 ts 1778124092) — 자동 봇 메시지 매일 발송 시작
  - **4월 매체 정산서 client 전달** (홍유니 5/7 08:00 ts 1778108404) — 대행수수료/광고비 분리
  - **4월 세금계산서 발행 결재상신** (하지현 5/7 15:47) — 황아인 5/8 발행 진행
  - **메타 캠페인 운영 점검 요청** (이동호 5/7 12:00) — Retargeting 일 $500 비효율 + 이벤트별 신규 캠페인 통합 점검
  - **5/15 AI Sachet 디자인 1차 (Vicky figma)** (5/7 18:18 황아인 thread) — 이동호 반영 완료
  - **5/7 EOD bot thread reply (홍유니)** — 시트 task 1차 정리, 잔여는 황아인/김소형 정리, 이동호 task는 홍유니 today 추가 확인
- **시트 미반영 신규 task (사용자 추가 필요)** 메시지 본문에 명시:
  - Primer Launch Deal 종료 sub-task 2건 (이동호 Shopify 롤백 5/28 KST 16:00 + 홍유니 광고 OFF 5/28 KST 15:00)
  - Glow Gift Festival 종료 sub-task 2건 (이동호 5/30 16:00 + 홍유니 5/30 15:00)
  - 5/15 AI Sachet sub-task 8건 (No Joke Sale 패턴, 5/13~5/15)
  - 프라이머 후속 번들 2종 검증 (이동호 5/9)
  - 6/1~10 V포토북 이벤트 + 5/25~ 티징 (자연님 기획안 5/4 ts 1777538236)
  - 메타 상시 소재 3종 기획안+제작 (Vicky/김소형, 5/7 12:00 / 5/29)
  - AI 비디오 영상 2종 검토 + 피드백 (황아인/김소형, 5/8)
  - Klaviyo Flow vs Madup Campaign R&R 응답안 (이동호/홍유니, 5/7)
- **시트 미반영 status 변경 (사용자 직접 수정 필요)**:
  - R158/R159/R160/R163 Primer Launch Deal sub-task → Complete (4/27 라이브 완료)
  - R176/R177/R178/R181 Glow Gift Festival sub-task → Complete (5/1 라이브 완료)
  - R208 Meta Ads Audience Segments → Complete (이동호 4/25 직접 세팅)
  - R165 Klaviyo Notion 적재 → 5/4 시트 일원화 결정으로 종결 권장
- RC 위반: 0건 (사전 grep CLEAN, 주간 경계 명시, draft task 시트 status 재조회 완료)
- 발송 ts: 1778199540.852299 (C0ALWLLQFU7 내부 채널, 확인 없이 자동 발송)
- 운영 규칙 변경 신규 (사용자 신규 cadence 합의는 봇 5/6 EOD에 이미 반영됨, 별도 추가 없음):
  - 메타 상시 소재 cadence: 매월 1회 → 2주 1회 텍스트 / 6월 이미지 교체 (자연님 5/6 18:56 합의)
  - Klaviyo Flow vs Campaign R&R 분기 (자연님 5/4 18:26 ts 1777886795 Retentics 도입 공지)
  - Klaviyo 발송 일정·문구 시트 일원화 (이동호 5/4 12:46)
  - 셰이드 옵션 제품 더블 체크 + 크로스체크 의무화 (5/6 인종차별 CS 2건 후)
