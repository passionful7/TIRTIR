...
### 5/22 audit (cloud container)
- 시작 시각: 2026-05-22 09:00 KST 추정 (금요일)
- 종료 시각: 2026-05-22 09:18 KST
- 트리거: 사용자 `/tirtir-audit` skill 호출 ("organize tasks for the TIRTIR campaign"), Claude Code on the web 세션
- 환경 제약: **gws CLI 인증 실패 (invalid_grant)** — 시트 직접 update 불가. Drive MCP로 read-only 가능. Slack 발송은 SLACK_TOKEN으로 정상 동작.
- 4/22 audit 이후 30일 경과 (그 사이 5/1, 5/4, 5/5, 5/6, 5/7, 5/8, 5/11, 5/12, 5/13, 5/14, 5/18 EOD 봇 메시지 발송됨 — 매도비 봇 자동화는 5/7부터 leverdashboard.com/tirtir와 연동되어 가동 중)
- 주간 경계: today=5/22(금) → 이번주 5/18(월)~5/24(일), 다음주 5/25(월)~5/31(일)
- 5/18 EOD(ts 1779064115) 이후 4일분 incremental update
- Sub-agent 2개 병렬 실행: 내부 채널 4/14~5/21 스캔 + 고객사 채널 4/20~5/21 스캔
- 신규 발견 (Slack 추적):
  - **완료 (5/18 이후)**: 프라이머 신번들 메타 광고 소재 2종 (김소형 5/21) / V Mini Photobook Shopify 리스팅 (황아인 5/21) / 써머빅세일 티저+딜페이지 디자인 (황아인 5/21) / 5/20~30 할인율 1차 Shopify 적용 (이동호 5/19) / Glow Gift 3차 Klaviyo 발송일자 변경
  - **신규 프로모션**: V Mini Photobook 5/25 사전 라이브 → 6/10 종료 (자연님 5/4 ts 1777538236 기획 + 5/21 13:57 ts 1778885830 종료페이지 컨펌 요청)
  - **자연님 5/21 신규 요청 5건**: (1) 프라이머 메타 신소재 라이브 (ts 1778907062), (2) V포토북 종료 페이지 6/5까지 프리뷰 (ts 1778885830), (3) 메인페이지 MMM 모듈 2건 누락 진단 (ts 1778877356), (4) 김현서 답변 리마인더 (ts 1778906834), (5) Klaviyo 일정 충돌 (5/25 사전 + 5/26 Glow Gift 3차) (ts 1778896030)
  - **운영 규칙 변경 (5/13~)**: 결제 방식 변경 (6/1부터 매드업 직결제, 5/31 마지막 매드업 카드 결제, 수수료=(순매출-매체비)×4%) / 6-7월 매출/광고 예산 변경 (7월→6월 당김) / 6/17~30 2차 할인율 조정 예약 (AI 28%, 컨실러 20%)
- **시트 update 필요 (gws 인증 복구 후 수동 처리 필요)**: 
  - 신규 행 다수 (V포토북 sub-task 4건 + 자연님 5/21 신규 요청 5건 + 5/26 유튜브 인플루언서 광고 시작 task + 5/31 결제 방식 전환 task)
  - Status 변경: R20/R21 Processing 유지(부분 답변 후 client 대기) / R3·R12 Processing 유지(5월 본격) / R14·R15·R32 HOLDING 명시화 필요(자연님 5/8 결정)
  - Result 업데이트: R10 (PDP 아마존 A+ 진행) / R16 (5/15 카탈로그 반영) / R29 (5/18~ 본격 운영 예정) 등 30+ 활성 task의 ts 갱신
- **RC 위반: 0건** (gws 제약 미리 발견 → workaround 적용. 발송 전 grep CLEAN)
- 발송 ts: 1779409092.523799 (C0ALWLLQFU7 내부 채널, 확인 없이 자동 발송)
- **이번 audit 후 follow-up**: 사용자가 시트 동기화 진행 (gws 인증 복구 또는 수동 반영) — 시트 update list는 Slack 메시지 본문 + 본 AUDIT_LOG 진행 노트로 충분히 추적 가능
