...
### 5/4 09:35 audit
- 시작 시각: 2026-05-04 09:25 KST (월요일)
- 종료 시각: 2026-05-04 09:38 KST
- 트리거: 사용자 `/tirtir-audit` skill 호출, "organize tasks for the TIRTIR campaign"
- 주간 경계: today=5/4(Mon) → 이번주 5/4(월)~5/10(일), 다음주 5/11(월)~5/17(일)
- 4/22 audit 이후 ~12일 경과 (장기 gap, 활동 매우 많음)
- 실행 방식: 양 채널 sub-agent 2개 병렬 실행 (client + internal). Bot EOD thread 7건 + 모든 active task ts thread 양 채널 재독.
- 신규 발견:
  - **Complete 다수 (시트 미반영 — gws auth 만료)**:
    - R157~R163 Primer Launch Deal 4/27 라이브 완료 (KST 13:00, sub-task 일괄)
    - R175~R181 5/1 Glow Gift Festival 라이브 완료 (KST 13:00)
    - R208 Meta Audience Segments 이동호 직접 세팅 (4/25 15:46, 하지현 대행 불필요)
    - Spring Matcha Refresh 4/27 종료: 광고 OFF 황아인 4/28 14:50 + Shopify 롤백 이동호 4/28 16:15
    - Matcha Giveaway 4/24 종료 + Shopify OFF (이동호 4/25 16:01)
  - **신규 13건**: AO 광고 5월 개선 plan(홍유니, 5/7), Shop all 메뉴 정렬(이동호, 5/6), Pore Blur Set US Sold Out 노출 변경(이동호, today), 5-6월 인플루언서 파트너광고(홍유니, 5/26), 5월 미디어믹스 시트 양식(황아인, 5/4), Meta EU top7 언어 테스트(황아인, 5/2), Pmax 추가 텍스트 에셋(김소형, 5/2), AI 샤셋 데일리 진행 공유(김소형), AI 샤셋 1개 우선 리스팅(황아인, 5/2), GLOW BASE SET 번들 오류 사후 처리(황아인/박서우, today), Pixer 색상/가격 컨펌(홍유니, today), Shopify→GS 자동연동(이동호, 5/2), Glow Gift 캐시 모니터링(이동호, today), 번들 크로스체크 SOP 문서화(이동호/황아인, 5/9)
  - **운영 규칙 변경 4건** (Slack draft 본문에 별도 섹션으로 통보):
    - UTM 통일 — Final URL Suffix 방식 (이동호 4/22 ts 1777246073)
    - Glow Gift Festival 운영기간/할인율 SWAP — 5/1~5/29(29일), 4단→3단(20%/25%/30%), Klaviyo 발송 5/1·5/13·5/25 (자연님 4/23 ts 1776942192)
    - Klaviyo 레퍼런스 사진 저작권 정책 — 외부 레퍼 그대로 사용 금지 (자연님 4/23 19:21)
    - Klaviyo SHOP NOW 버튼 컨벤션 — 4/28 이후 캠페인부터 이미지 형태 (박서우 4/28 17:27)
    - 소재 기획→시안 컨펌 프로세스 단순화 — 디자인팀 1회 sync로 종결 (홍유니 4/29 ts 1777457201)
- **RC 위반 / 예방**:
  - RC21 사전 감지: Draft 본문에 "CLAUDE.md 반영 필요" 1건 → grep 단계에서 즉시 제거 후 재grep CLEAN. 발송 전 예방 성공
- **🚫 INFRA BLOCKER (CRITICAL)**: gws CLI auth refresh token 만료 → 시트 batchUpdate **불가**
  - 증상: `gws auth login` 실패 ("invalid_grant: Token has been expired or revoked")
  - 영향: 신규 13건 task append 불가, Complete 처리 불가 (Primer R157~R163, Glow Gift R175~R181, R208), Result 업데이트 불가
  - 우회: Slack draft에 모든 변경사항 명시 + 시트 링크 footer로 안내
  - 후속 조치 필요: 사용자가 `/root/.config/gws/credentials.json` 재발급 또는 `gws auth setup` 후 재실행 → 시트 일괄 update 필요
  - **다음 audit 시작 전 반드시 시트와 본 audit의 변경사항 reconcile 필요** (RC20 risk: 직전 EOD 메시지 reuse 시 stale carry-over 가능)
- 발송 ts: 1777855110.671849 (C0ALWLLQFU7 내부 채널, 확인 없이 자동 발송)
- 발송 시점: 5/4 09:38 KST. Primer Launch Deal 종료 P0 (광고 OFF KST 15:00 / Shopify 롤백 KST 16:00, 약 5h 25min 후 D-day) 임박 — 시트 update보다 메시지 발송 우선 처리
